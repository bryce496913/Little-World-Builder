...
struct SavedIsland: Codable, Identifiable {
    var id: UUID
    var name: String
    var createdAt: Date
    var updatedAt: Date
    var baseIslandAssetFileName: String
    var waterType: IslandWaterType
    var placedAssets: [SavedIslandAsset]
    var animationMetadata: IslandAnimationMetadata
    var scale: CodableVector3
    var thumbnailFileName: String?
}

struct SavedIslandAsset: Codable, Identifiable {
    var id: UUID
    var assetFileName: String
    var displayName: String
    var category: ModelCategory
    var relativePosition: CodableVector3
    var relativeRotation: CodableQuaternion
    var relativeScale: CodableVector3
    var animationState: String?
}

enum IslandWaterType: String, Codable, CaseIterable {
    case none
    case calm
    case ocean
    case pond
...
struct IslandAnimationMetadata: Codable {
    var isEnabled: Bool
    var notes: String?

    static let empty = IslandAnimationMetadata(isEnabled: false, notes: nil)
}
...
struct CodableQuaternion: Codable, Equatable {
    ...
    init(_ value: simd_quatf) {
        self.init(x: value.vector.x, y: value.vector.y, z: value.vector.z, w: value.vector.w)
    }
...
struct CodableTransform: Codable, Equatable {
    ...
    var realityKitTransform: Transform {
        Transform(scale: scale.simd, rotation: rotation.simd, translation: position.simd)
    }
}
